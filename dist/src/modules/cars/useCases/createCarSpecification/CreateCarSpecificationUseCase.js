...
let CreateCarSpecificationUseCase = class CreateCarSpecificationUseCase {
    ...
    execute({ id, specifications }) {
        return __awaiter(this, void 0, void 0, function* () {
            const carExists = yield this.carsRepository.findById(id);
            if (!carExists) {
                throw new AppError_1.AppError("Car Not Exists!");
            }
            const findedSpecifications = yield this.specificationsRepository.findByIds(specifications);
            const car = yield this.carsRepository.updateSpecifications({
                id,
